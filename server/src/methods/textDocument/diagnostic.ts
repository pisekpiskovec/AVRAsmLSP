...
export const diagnostic = (message: RequestMessage): FullDocumentDiagnosticReport | null => {
  ...
  const lines = content.split("\n");
    ...
    const message = wordSuggestions.length ? `${invalidWord} isn't valid instruction in the dictionary. Did you mean ${wordSuggestions.join(", ")}` : `${invalidWord} isn't valid instruction in the dictionary.`
